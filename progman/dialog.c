...
DIALOG_NEW_DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    pNewItem = (PNEW_ITEM_CONTEXT)GetWindowLong(hDlg, 8);
...
DIALOG_COPY_MOVE_DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    pCopyMove = (PCOPY_MOVE_CONTEXT)GetWindowLong(hDlg, 8);
...
DIALOG_GROUP_DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    pGroupAttributes = (PGROUP_ATTRIBUTES_CONTEXT)GetWindowLong(hDlg, 8);
...
BOOL
CALLBACK
EnumPickIconResourceProc(HMODULE hModule, LPCWSTR lpszType, LPWSTR lpszName, LONG lParam)
{
    HICON hIcon;
    ...
    if (IS_INTRESOURCE(lpszName))
        swprintf(szName, L"%u", (unsigned)(UINT_PTR)lpszName);
    else
...
DIALOG_SYMBOL_DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    pIconContext = (PPICK_ICON_CONTEXT)GetWindowLong(hDlg, 8);
    ...
    switch (uMsg)
    {
        case WM_INITDIALOG:
        {
            ...
            pIconContext->hLibrary = LoadLibraryExW(pIconContext->szName, NULL, DONT_RESOLVE_DLL_REFERENCES | LOAD_LIBRARY_AS_DATAFILE);
            if (pIconContext->hLibrary)
            {
                EnumResourceNamesW(pIconContext->hLibrary,
                                   (LPCWSTR)RT_GROUP_ICON,
                                   EnumPickIconResourceProc,
                                   (LONG)pIconContext->hDlgCtrl);
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam))
            {
                case PM_BROWSE:
                {
                    ...
                    pIconContext->hLibrary = LoadLibraryExW(filename, NULL, LOAD_LIBRARY_AS_DATAFILE);
                    if (pIconContext->hLibrary)
                    {
                        EnumResourceNamesW(pIconContext->hLibrary,
                                           (LPCWSTR)RT_GROUP_ICON,
...
DIALOG_PROGRAM_DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    pProgramAttributes = (PPROGRAM_ATTRIBUTES_CONTEXT)GetWindowLong(hDlg, 8);
...
DIALOG_EXECUTE_DlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    pExecuteContext = (PEXECUTE_CONTEXT)GetWindowLong(hDlg, 8);
